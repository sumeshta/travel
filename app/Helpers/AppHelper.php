...
function service_price_currency_code($row = null)
{
...
function service_price_currency_options()
{
    return Currency::getAdminCurrencyOptions();
}

function apply_service_price_currency_to_request(\Illuminate\Http\Request $request, array $fields = ['price', 'sale_price'])
{
    $currency = strtolower((string) $request->input('price_currency', setting_item('currency_main', 'usd')));
    $request->merge(['price_currency' => $currency]);
    ...
    foreach ($fields as $field) {
        ...
        $request->merge([
            $field => Currency::convertFromCurrencyToMain((float) $value, $currency),
        ]);
    }
}
...
function get_request_route_segments()
{
    $segments = request()->segments();

    if (setting_item('site_enable_multi_lang') && class_exists(\Modules\Language\Models\Language::class)) {
        $localeCodes = \Modules\Language\Models\Language::getActive()->pluck('locale')->all();
        if (!empty($segments[0]) && in_array($segments[0], $localeCodes, true)) {
...
function get_service_detail_route_prefixes()
{
    ...
    $prefixes = array_values(array_filter([
        config('tour.tour_route_prefix'),
        config('hotel.hotel_route_prefix'),
        config('car.car_route_prefix'),
        config('space.space_route_prefix'),
        config('event.event_route_prefix'),
        config('boat.boat_route_prefix'),
        config('news.news_route_prefix'),
        config('flight.flight_route_prefix'),
    ]));

    return $prefixes;
}

function is_service_detail_page()
{
    [$prefix, $slug] = get_request_route_segments();

    return !empty($prefix) && !empty($slug) && in_array($prefix, get_service_detail_route_prefixes(), true);
}

function get_site_share_image()
{
    $logoId = setting_item('logo_id');
    if (!empty($logoId)) {
        $url = get_file_url($logoId, 'full');
        if (!empty($url)) {
            return $url;
        }
    }
    ...
    if (!empty($favicon)) {
        $url = get_file_url($favicon, 'full');
        if (!empty($url)) {
            return $url;
        }
    }

    return url('images/favicon.png');
}

function get_service_share_image($row = null)
{
    if (empty($row)) {
        return get_site_share_image();
    }

    if (method_exists($row, 'getSeoMeta')) {
        $meta = $row->getSeoMeta();
        if (!empty($meta['seo_share']['facebook']['image'])) {
            $url = get_file_url($meta['seo_share']['facebook']['image'], 'full');
            if (!empty($url)) {
                return $url;
            }
        }
    ...
    if (method_exists($row, 'getGallery')) {
        $gallery = $row->getGallery(true);
        if (!empty($gallery[0]['large'])) {
            return $gallery[0]['large'];
        }
        if (!empty($gallery[0]['thumb'])) {
...
function get_country_iso_by_name($name)
{
    ...
    if ($lookup === null) {
        $lookup = [];
        foreach (get_country_lists() as $code => $countryName) {
            $lookup[strtolower($countryName)] = strtolower($code);
        }
        $aliases = [
            'usa' => 'us',
            'u.s.a.' => 'us',
    ...
    if (strlen($key) === 2 && isset(get_country_lists()[strtoupper($key)])) {
        return $key;
    }
...
function get_location_country_info($location)
{
    if (empty($location)) {
        return null;
    }
    ...
    while ($current && $guard < 10) {
        $translation = method_exists($current, 'translate') ? $current->translate() : null;
        $name = trim($translation->name ?? $current->name ?? '');
        if ($name !== '') {
            $names[] = $name;
        }
    ...
    foreach ($names as $name) {
        if (strpos($name, ',') !== false) {
            ...
            foreach (array_reverse($parts) as $part) {
                $code = get_country_iso_by_name($part);
                if ($code) {
                    return [
                        'name' => get_country_name(strtoupper($code)),
                        'code' => $code,
                    ];
                }
            }
        ...
        $code = get_country_iso_by_name($name);
        if ($code) {
            return [
                'name' => get_country_name(strtoupper($code)),
                'code' => $code,
        ...
        $regionCode = config('location.region_country_map.' . strtolower($name));
        if ($regionCode) {
            $regionCode = strtolower($regionCode);
            return [
                'name' => get_country_name(strtoupper($regionCode)),
                'code' => $regionCode,
            ];
        }
    }
...
function get_location_display_parts($location)
{
    if (empty($location)) {
        return ['city' => '', 'country' => null];
    }

    $translation = method_exists($location, 'translate') ? $location->translate() : null;
    $fullName = trim($translation->name ?? $location->name ?? '');
    $country = get_location_country_info($location);
    $city = $fullName;

    if ($fullName !== '' && strpos($fullName, ',') !== false) {
        $parts = array_map('trim', explode(',', $fullName));
        $city = $parts[0];
    } elseif ($country && strcasecmp($fullName, $country['name']) === 0) {
        $city = '';
    }
...
if (!function_exists('bookable_external_url')) {
    function bookable_external_url($service, string $field): ?string
    {
        if (!is_object($service)) {
            return null;
        }

        $url = $service->{$field} ?? null;
        if ($field === 'website' && empty(trim((string) $url)) && !empty($service->author)) {
            $url = $service->author->website ?? null;
        }
