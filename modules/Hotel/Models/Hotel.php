...
class Hotel extends Bookable
{
    ...
    {
        return $this->hasOne("Modules\Hotel\Models\HotelCategory", "id", 'category_id')->with(['translation']);
    }
    ...
    public function getPriceTariffs()
    {
        ...
        foreach ($this->rooms as $room) {
            ...
            $res[] = [
                ...
                'price'         => $room->price,
                'price_html'    => format_money($room->price) . '<span class="unit">/' . __('per night') . '</span>',
                'size_html'     => $room->size ? size_unit_format($room->size) : '',
                'beds_html'     => $room->beds ? 'x' . $room->beds : '',
                'adults_html'   => $room->adults ? 'x' . $room->adults : '',
                'children_html' => $room->children ? 'x' . $room->children : '',
    ...
    public function search($request)
    {
        ...
        if (!empty($price_range = $request["price_range"] ?? "")) {
            ...
            $priceSql = 'CASE WHEN IFNULL(bravo_hotels.sale_price, 0) > 0 THEN bravo_hotels.sale_price ELSE bravo_hotels.price END';
            $model_hotel->where(function ($query) use ($pri_from, $pri_to, $priceSql) {
                $query->whereRaw("({$priceSql}) BETWEEN ? AND ?", [$pri_from, $pri_to])
                    ->orWhereRaw("({$priceSql}) IS NULL");
            });
        }
        ...
        return $model_hotel->with(['location.parent','hasWishList','translation','termsByAttributeInListingPage']);
    }
