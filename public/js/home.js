...
jQuery(function ($) {
    ...
    function isExternalSlideUrl(url) {
        url = normalizeSlideUrl(url);
    ...
    function goToSlideUrl(url) {
        url = normalizeSlideUrl(url);
        if (!url) {
            return;
        }
        if (isExternalSlideUrl(url)) {
    ...
    function initHomeSliderClickArea($root) {
        $root.find(".effect").on("click", function (e) {
            if ($(e.target).closest(".g-form-control, .bravo_form, input, select, textarea, button, label, a.bravo-slider-hit-area").length) {
                return;
            }
            var $slide = $root.find(".owl-item.active .owl-home-slide").first();
            if (!$slide.length) {
                $slide = $(e.target).closest(".owl-home-slide");
            }
            var url = $slide.attr("data-slide-url") || $slide.find("a.bravo-slider-hit-area").attr("href");
            goToSlideUrl(url);
        });

        $root.find(".effect").on("click", "a.bravo-slider-hit-area", function (e) {
    ...
    function initHomeSliderCarousel($root) {
        ...
        initHomeSliderClickArea($root);
    }

    $(".bravo-form-search-slider").each(function () {
        initHomeSliderCarousel($(this));
    });
    ...
    $('.bravo-form-register [type=submit]').click(function (e) {
        ...
        $.ajax({
            ...
            'data': {
                ...
                'phone': (window.BravoPhoneIntl && BravoPhoneIntl.getValue(form.find('input[name=phone]')[0])) || form.find('input[name=phone]').val(),
    ...
    function fitMobileMenuGrid() {
        var $menus = $('.bravo-menu .main-menu.menu-with-icons');
        if ($(window).width() > 768) {
            $menus.css('grid-template-columns', '');
            return;
        }
        $menus.each(function () {
            var count = $(this).children('li.depth-0').length;
            if (count <= 0) {
                return;
            }
            var cols = count <= 6 ? count : Math.ceil(count / 2);
            $(this).css('grid-template-columns', 'repeat(' + cols + ', minmax(0, 1fr))');
        });
    }
    fitMobileMenuGrid();
    $(window).on('resize orientationchange', fitMobileMenuGrid);

    $(document).on('click', '.bravo-menu .menu-with-icons > li.depth-0 > a', function (e) {
        if ($(window).width() > 768) {
            return;
        }
        var $li = $(this).closest('li');
        if (!$li.children('.children-menu').length) {
            return;
        }
        e.preventDefault();
        $li.siblings('.menu-open').removeClass('menu-open');
    ...
    $(document).on('click', function (e) {
        ...
        if (!$(e.target).closest('.bravo-menu .menu-with-icons > li.depth-0').length) {
            $('.bravo-menu .menu-with-icons > li.depth-0.menu-open').removeClass('menu-open');
        }
    });
    ...
    $('.btn-submit-enquiry').click(function (e) {
        ...
        if (window.BravoPhoneIntl) {
            BravoPhoneIntl.syncForm(form[0]);
        }
