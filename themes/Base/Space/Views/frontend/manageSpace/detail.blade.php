...
@section('content')
    <h2 class="title-bar no-border-bottom">
        {{$row->id ? __('Edit: ').$row->title : __('Add new Travel agent')}}
    </h2>
        ...
        <form action="{{route('space.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post">
            ...
            <div class="form-add-service">
                <div class="nav nav-tabs nav-fill" id="nav-tab" role="tablist">
                    ...
                    @endif
                    <a data-toggle="tab" href="#booking-link" aria-selected="false">6. {{ __('Website & Booking') }}</a>
                </div>
                <div class="tab-content" id="nav-tabContent">
                    ...
                    @endif
                    <div class="tab-pane fade" id="booking-link">
                            @include('Space::admin/space/booking_url')
                        </div>
                </div>
            </div>
...
@push('js')
    ...
    @include('Location::frontend.vendor.places-script')
@endpush
